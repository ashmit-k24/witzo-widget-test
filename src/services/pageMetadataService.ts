...
// extractAsync classifies pages and updates Pinecone vector metadata in the background.
...
export function extractAsync(
	...
	(async () => {
		...
		for (const chunk of chunks) {
			if (chunk.isHype) continue; // skip HyPE vectors
		...
		logger.warn("pageMetadataService: async extraction failed", { err });
	});
}
