...
class ScraperService {
     ...
     private isValidInternalUrl(url: string, baseHostname: string): boolean {
          try {
     ...
     async countAvailablePages(
          ...
          logger.info(`Starting page count for ${url}`, { maxDepth, maxPages });
          ...
          while (queue.length > 0 && discoveredUrls.size < maxPages) {
               ...
               try {
                    ...
                    const response = await axios.get(current.url, {
                         ...
                         maxRedirects: 3,
                         headers: {
                              "User-Agent": "Mozilla/5.0 (compatible; WitzoBot/1.0; +https://witzo.ai)",
                         },
                    });
                    ...
                    $("a[href]").each((_, element) => {
                         if (discoveredUrls.size >= maxPages) return false;

                         const href = $(element).attr("href");
                         if (!href) return true;

                         const normalizedUrl = this.normalizeUrl(href, current.url);
                         if (!normalizedUrl) return true;

                         if (
                              !discoveredUrls.has(normalizedUrl) &&
                              this.isValidInternalUrl(normalizedUrl, baseHostname)
                         ) {
                    ...
                    logger.debug(`Failed to fetch ${current.url} during page count`, { error });
               }
          }

          const elapsedTime = Date.now() - startTime;
          const estimatedScrapingTime = Math.ceil((discoveredUrls.size * 2) / 60); // ~2 seconds per page

          logger.info(`Page count completed for ${url}`, {
          ...
          return {
               totalPages: discoveredUrls.size,
               discoveredUrls: Array.from(discoveredUrls).slice(0, 50), // Return first 50 URLs as sample
               baseUrl,
