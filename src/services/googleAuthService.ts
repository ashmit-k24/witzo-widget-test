    ...
    try {
      ...
      if (userResult.rows.length === 0) {
        ...
        const insertResult = await client.query<User>(
          ...
          [newUserId, normalizedEmail, profile.verified_email]
        );
        userId = insertResult.rows[0].id;
        userResult = insertResult;
        isNewUser = true;
        logger.info('New user created via Google OAuth', { email: normalizedEmail, userId });
      } else {
        // User exists
        userId = userResult.rows[0].id;

        // Update user as verified if Google email is verified
        if (profile.verified_email && !userResult.rows[0].is_verified) {
          await client.query(
        ...
        logger.info('Existing user logged in via Google OAuth', { email: normalizedEmail, userId });
      }
      ...
      user.is_verified = profile.verified_email || user.is_verified;
