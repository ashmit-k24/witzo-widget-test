...
class DocumentParserService {
     async parseDocument(filePath: string, filename: string, fileSize: number): Promise<ParsedDocument> {
     ...
     private async parsePDF(filePath: string): Promise<string> {
          try {
               ...
               logger.info("PDF buffer read successfully", {
               ...
               if (!result.text || result.text.trim().length === 0) {
                    throw new Error("PDF parsing succeeded but no text was extracted. The PDF might be scanned images or password-protected.");
               }
     ...
     private async parseExcel(filePath: string): Promise<string> {
          try {
               ...
               if (!content || content.trim().length === 0) {
                    throw new Error("Excel file parsing succeeded but no content extracted");
               }
     ...
     async processAndStoreDocument(
          ...
          try {
               logger.info(`Processing document: ${filename} for user: ${userId}`);

               const parsedDoc = await this.parseDocument(filePath, filename, fileSize);
               ...
               const estimatedChunks = Math.ceil(parsedDoc.content.length / 8000);

               logger.info(`Document processed successfully: ${filename} (${estimatedChunks} chunks)`);
               ...
               logger.error("Error processing document", { error, filename, userId });
               throw error;
          } finally {
