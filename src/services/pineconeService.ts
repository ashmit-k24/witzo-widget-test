...
import { PineconeMetadata, ScraperUsageStats, SCRAPER_PAGE_LIMITS } from "../types";
import {
...
class PineconeService {
  ...
  private async forEachUserRecord(
    userId: string,
    handler: (
      records: Record<string, PineconeRecord<any>>,
    ) => Promise<void> | void,
  ): Promise<void> {
    ...
    do {
      ...
      paginationToken = listResponse.pagination?.next || undefined;
  ...
  async deleteDocumentsByUrl(userId: string, url: string): Promise<void> {
    try {
      ...
      let matchFn: (recordUrl?: string) => boolean;
      let logLabel: string;

      if (url.startsWith("document://")) {
        logLabel = url;
        matchFn = (recordUrl?: string) => recordUrl === url;
        logger.info(`Deleting document: ${logLabel} (user: ${userId})`);
      } else {
        let baseUrl: string;
        try {
        ...
        matchFn = (recordUrl?: string) =>
          Boolean(recordUrl && recordUrl.startsWith(baseUrl));
        logger.info(
      ...
      await this.forEachUserRecord(userId, async (records) => {
        for (const [id, record] of Object.entries(records)) {
          const recordUrl =
            (record.metadata?.url as string | undefined) ?? undefined;
      ...
      for (const batch of this.chunkArray(matchingIds, 1000)) {
        await pineconeCircuitBreaker.execute(async () => {
      ...
      logger.error("Error deleting documents from Pinecone", {
        error,
        url,
        userId,
      });
  ...
  async deleteAllUserDocuments(userId: string): Promise<void> {
    try {
      ...
      logger.error("Error deleting all user documents from Pinecone", {
        error,
        userId,
      });
      throw error;
  ...
  async checkSourceExists(
    userId: string,
    sourceUrl: string,
  ): Promise<{ exists: boolean; chunks: number; scrapedAt?: string }> {
    try {
      const namespace = this.getUserNamespace(userId);
      const index = this.pinecone.index(this.indexName).namespace(namespace);
      ...
      const sanitizedUserId = this.sanitizeId(userId);
      const sanitizedUrl = this.sanitizeId(urlToCheck);
      ...
      do {
        ...
        if (ids.length > 0) {
          ...
          if (!scrapedAt) {
            const fetchResponse = await index.fetch([ids[0]]);
            const firstRecord = Object.values(fetchResponse.records ?? {})[0];
            if (firstRecord?.metadata) {
              const metadata = firstRecord.metadata as unknown as PineconeMetadata;
      ...
      logger.error("Error checking if source exists", { error, userId, sourceUrl });
      throw error;
    }
  }
  ...
  async getAllUserSources(userId: string): Promise<{
    ...
    try {
      ...
      await this.forEachUserRecord(userId, async (records) => {
        for (const record of Object.values(records)) {
          const metadata = record.metadata as
            | (PineconeMetadata & { fileType?: string; uploadedAt?: string })
            | undefined;
      ...
      logger.error("Error fetching user sources from Pinecone", {
        error,
        userId,
      });
      throw error;
  ...
  async getScrapedWebsiteCount(userId: string): Promise<number> {
    try {
      const sources = await this.getAllUserSources(userId);
      // Count unique website URLs (not documents)
      return sources.websites.length;
    } catch (error) {
      logger.error("Error counting scraped websites", { error, userId });
      throw error;
    }
  }
  ...
  async getScraperUsageStats(
    ...
    const pagesUsed = await this.getScrapedWebsiteCount(userId);
    const pagesLimit = SCRAPER_PAGE_LIMITS[planType];
  ...
  async canUserScrape(
    ...
    const usage = await this.getScraperUsageStats(userId, planType);
