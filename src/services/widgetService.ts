...
class WidgetService {
  ...
  async verifyWidgetKey(
    ...
    try {
      ...
      const normalizedReferer = refererDomain?.toLowerCase();
      ...
      if (widget.allowed_domains && widget.allowed_domains.length > 0) {
        if (!normalizedReferer) {
          return {
            valid: false,
            message: "Missing referer for domain-restricted widget",
          };
        }
        ...
        const isAllowed = widget.allowed_domains.some((domain) => {
          ...
          if (sanitizedDomain.startsWith("*.")) {
            const baseDomain = sanitizedDomain.slice(2);
            return (
              normalizedReferer === baseDomain ||
              normalizedReferer.endsWith(`.${baseDomain}`)
            );
          }

          return (
            normalizedReferer === sanitizedDomain ||
            normalizedReferer.endsWith(`.${sanitizedDomain}`)
          );
        });
  ...
  async trackWidgetEvent(
    ...
    eventType: string,
    eventData: any = {},
    metadata?: { ipAddress?: string; userAgent?: string; refererUrl?: string },
  ): Promise<void> {
    try {
  ...
  async flushAnalytics(): Promise<void> {
    try {
      ...
      if (!eventsStr || (Array.isArray(eventsStr) && eventsStr.length === 0))
        return;
      ...
      const events = (Array.isArray(eventsStr) ? eventsStr : [eventsStr])
        .map((s) => {
          try {
            ...
            logger.error("Failed to parse analytics event", {
              parseError,
              event: s,
            });
            return null;
      ...
      try {
        ...
        logger.error("Error flushing analytics, attempting re-queue", {
          error,
        ...
        try {
          ...
          logger.error("Failed to re-queue events - data loss may occur", {
            requeueError,
          });
        }
      } finally {
