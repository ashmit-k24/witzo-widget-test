...
const isValidUrl = (url: string, baseUrl: string): boolean => {
        try {
                ...
                if (excludeExtensions.some((ext) => urlObj.pathname.toLowerCase().endsWith(ext))) return false;
...
const extractPageData = (html: string, url: string): ScrapedPage => {
...
const processScrapeJob = async (job: Job<ScrapeJobData>) => {
        const { userId, url, maxDepth, maxPages } = job.data;
        const visitedUrls = new Set<string>();
        const urlQueue: Array<{ url: string; depth: number }> = [{ url, depth: 0 }];
        let pagesScraped = 0;

        logger.info(`Starting scrape job ${job.id} for user ${userId} on ${url}`);
        ...
        while (urlQueue.length > 0 && visitedUrls.size < maxPages) {
                ...
                // Report progress
                await job.updateProgress(Math.round((visitedUrls.size / maxPages) * 100));
                ...
                try {
                        ...
                        await pineconeService.upsertDocument(userId, pageData.url, pageData.title, pageData.content, pageData.metadata);

                        if (depth < maxDepth) {
                                for (const link of pageData.links) {
                                        const normalizedLink = normalizeUrl(link);
                                        if (!visitedUrls.has(normalizedLink) && isValidUrl(normalizedLink, url)) {
                                                urlQueue.push({ url: normalizedLink, depth: depth + 1 });
                                        }
                                }
                        }
                } catch (error) {
                        logger.error(`Error scraping ${normalizedUrl}`, { error });
                        job.log(`Failed to scrape ${normalizedUrl}: ${error instanceof Error ? error.message : String(error)}`);
                }
        }

        return { pagesScraped, visitedUrls: Array.from(visitedUrls) };
};
...
export const createScraperWorker = () => {
        ...
        worker.on("completed", (job) => {
                logger.info(`Job ${job.id} completed! Scraped ${job.returnvalue.pagesScraped} pages.`);
        });
