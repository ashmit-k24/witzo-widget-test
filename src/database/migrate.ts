 ...
 */
const addPricingFieldsToUsers = async (): Promise<void> => {
     const client: PoolClient = await pool.connect();
    ...
    `);
          ...
          if (columnCheck.rows.length === 0) {
               logger.info("Adding pricing fields to users table...");
      ...
      `);
               ...
               logger.info("Pricing fields added to users table successfully");
          } else {
      ...
      `);
          ...
          await client.query("ROLLBACK");
          const err = error as Error;
          logger.error("Error adding pricing fields to users", {
               error: err.message,
               stack: err.stack,
          });
          throw error;
...
export { addPricingFieldsToUsers, createTables, migrateSessionsTable };
