...
router.get(
	...
	async (_req: Request, res: Response) => {
		...
		health.circuitBreakers = {
			...
			pinecone:
				pineconeCircuitBreaker.getMetrics(),
		};
...
router.get(
	...
	async (_req: Request, res: Response) => {
		try {
			...
			metrics.push(
				`# HELP db_pool_waiting Waiting database connections in pool`,
			);
			metrics.push(
			...
			const stateValue =
				openAIMetrics.state === "CLOSED"
					? 0
					: openAIMetrics.state === "HALF_OPEN"
						? 1
						: 2;
			metrics.push(
				`circuit_breaker_state{service="openai"} ${stateValue}`,
			);

			const pineconeMetrics =
				pineconeCircuitBreaker.getMetrics();
			const pineconeStateValue =
				pineconeMetrics.state === "CLOSED"
					? 0
					: pineconeMetrics.state === "HALF_OPEN"
						? 1
						: 2;
			metrics.push(
				`circuit_breaker_state{service="pinecone"} ${pineconeStateValue}`,
			);
