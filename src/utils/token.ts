...
class TokenUtil {
  ...
  constructor() {
    ...
    this.accessTokenExpiry = config.ACCESS_TOKEN_EXPIRY_MINUTES;
    this.refreshTokenExpiry = config.REFRESH_TOKEN_EXPIRY_DAYS;
  }
  ...
  generateAccessToken(payload: Omit<TokenPayload, 'type'>): {
    ...
    const expiresAt = new Date(Date.now() + this.accessTokenExpiry * 60 * 1000);
  ...
  generateRefreshToken(payload: Omit<TokenPayload, 'type'>): {
    ...
    const expiresAt = new Date(Date.now() + this.refreshTokenExpiry * 24 * 60 * 60 * 1000);
  ...
  verifyToken(
    ...
    try {
      ...
      const payload = JSON.parse(payloadString) as TokenPayload & { exp: number; iat: number };
