...
export class WitzoChatWidget extends HTMLElement {
	...
	connectedCallback() {
		...
		this.apiBaseUrl =
			...
			this.apiUrl.replace("/api/v1/webhook", "");
		this.widgetKey =
		...
		this.ratingShown = session.getRatingShown(
			this.sessionId,
		);
		this.ratingSubmitted =
			session.getRatingSubmitted(this.sessionId);
		this._leadFormCompleted =
			session.getLeadFormCompleted(
				this.widgetKey,
				this.sessionId,
			);

		// 3. Init language preference
		const langKey = session.getLanguageKey(
			this.widgetKey,
		);
		const configuredLang =
			session.normalizeLanguage(
				this.config.defaultLanguage,
				SUPPORTED_LANGUAGES,
			) || "en";
		const storedLang = session.normalizeLanguage(
		...
		if (
			!document.getElementById("witzo-font-inter")
		) {
    ...
    `;
		...
		this.elements = {
			...
			floatingBtn: this.shadowRoot.getElementById(
				"floating-btn",
			),
			...
			cfName:
				this.shadowRoot.getElementById("cf-name"),
			cfEmail:
				this.shadowRoot.getElementById(
					"cf-email",
				),
			...
			langPillBtn: this.shadowRoot.getElementById(
				"langPillBtn",
			),
			langDropdown:
				this.shadowRoot.getElementById(
			...
			langItems: this.shadowRoot.querySelectorAll(
				".lang-dropdown-item",
			),
		};
		...
		if (
			session.isDailyLimitReached(this.widgetKey)
		) {
			setTimeout(
				() => this._lockSession(),
				this.config.primaryText ? 800 : 100,
			);
		}
	...
	toggleChat() {
		events.toggleChat(this);
	}
	handleLanguageSelect(code) {
		events.handleLanguageSelect(this, code);
	}
	...
	async handleSend() {
		...
		this._wasEndIntent =
			msg.isConversationEndMessage(text);
		...
		try {
			...
			if (
				...
				const result = await stream.consumeStream(
					response,
					(assembled) => {
						msg.updateStreamingBubble(
							typingEl,
							assembled,
							this.config.logoIcon,
							this,
				...
				if (result.donePayload?.sessionId)
					this._updateSession(
						result.donePayload.sessionId,
					);
				...
				this.appendBotReply(
					typingEl,
					result.assembled,
					result.donePayload?.assistantMessageId,
				);
				msg.appendSources(
					typingEl,
					result.donePayload?.sources,
				);
				if (result.donePayload?.calendlyBooking) {
					this.showCalendlyEmbed(
						result.donePayload.calendlyBooking,
					);
				}
				return;
			}
			...
			if (response.ok) {
				try {
					...
					if (result.sessionId)
						this._updateSession(result.sessionId);
					if (Array.isArray(result.sources))
						jsonSources = result.sources;
					if (result.calendlyBooking) {
						jsonSources = Array.isArray(
							result.sources,
						)
							? result.sources
							: [];
	...
	appendBotReply(typingEl, text, messageId) {
		...
		const dailyCount =
			session.incrementDailyCount(this.widgetKey);
		if (
			session.isDailyLimitReached(this.widgetKey)
		) {
	...
	showRatingAcknowledgement(rating) {
		...
		this.shadowRoot
			.querySelector(".rating-feedback-toast")
			?.remove();
	...
	handleMessageFeedbackClick(e) {
		...
		if (feedbackButton) {
			e.stopPropagation();
			const feedbackRoot = feedbackButton.closest(
				".message-feedback",
			);
			...
			if (
				feedbackButton.dataset.feedback === "down"
			) {
				const willOpen =
	...
	_getMessageFeedbackAcknowledgement(
		feedbackType,
		feedbackReason = null,
	) {
	...
	_upsertMessageFeedbackAcknowledgement(
		messageId,
		feedbackType,
		feedbackReason = null,
	) {
		...
		msg.updateBubble(
			wrapper,
			this._getMessageFeedbackAcknowledgement(
				feedbackType,
				feedbackReason,
			),
			this.config.logoIcon,
	...
	async submitMessageFeedback(
		...
		try {
			const response = await fetch(
				`${this.apiBaseUrl}/api/v1/widget/message-feedback`,
				{
					method: "POST",
					headers: {
			...
			this._upsertMessageFeedbackAcknowledgement(
				messageId,
				feedbackType,
				feedbackReason,
			);
	...
	showContactForm() {
		...
		if (!this._cfBound) {
			...
			if (this.elements.cfAgree) {
				this.elements.cfAgree.addEventListener(
					"change",
					() => this.updateCfSubmitState(),
				);
			}
		...
		try {
			...
			if (resp.ok) {
				if (this._serverLimitReached) {
					...
					this._setLeadFormCompleted(true);
					...
					this.updateSendButtonState();
					this._showTeamWillContactYouMessage();
					return;
				}
				if (this.config.leadFormEnabled) {
					this._setLeadFormCompleted(true);
	...
	_setLeadFormCompleted(value) {
		this._leadFormCompleted = Boolean(value);
		session.setLeadFormCompleted(
			this.widgetKey,
			this.sessionId,
			value,
		);
	...
	_showTeamWillContactYouMessage() {
		this.elements.contactFormSlot?.classList.add(
			"hidden",
		);
		this.elements.contactFormSlot?.classList.remove(
	...
	_hideHopeBanner() {
		...
		this.elements.messagesContainer
			?.querySelector(".chat-message.mt-space")
			?.classList.remove("mt-space");
	}
